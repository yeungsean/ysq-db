...
// stringLine ...
func (c *Cond) stringLine(ctx context.Context) string {
	if len(c.ops) == 0 && len(c.childCond) == 0 {
		return ""
	}
	sb := strings.Builder{}
	if len(c.ops) > 0 {
		sb.WriteString(c.ops[0].Column.String(ctx))
		if len(c.ops) == 1 {
			return sb.String()
		}

		for _, op := range c.ops[1:] {
			sb.WriteString(fmt.Sprintf("%s%s", op.LogicType, op.Column.String(ctx)))
		}
	}

	for _, child := range c.childCond {
...
// String ...
func (c *Cond) String(ctx context.Context) string {
	if len(c.ops) == 0 && len(c.childCond) == 0 {
		return ""
	} else if c.logic == Line {
		return c.stringLine(ctx)
	}
	...
	strs := ysq.FromSlice(c.ops).CastToStringBy(func(c *CondColumn) string { return c.Column.String(ctx) }).
	...
	cc := ysq.FromSlice(c.childCond).CastToStringBy(func(c *Cond) string { return c.String(ctx) }).
		Reduce("", func(s1, s2 string) string {
			if s1 == "" {
				return "(" + s2
			} else if s2 == "" {
